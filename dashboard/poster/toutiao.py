...
class TouTiao:
    def __init__(self, account, product_instance):
    ...
    def main(self, account, password, display_name):
        self._load_cookies()
        status, memo = self._post(display_name)
        return status, memo

    def _post(self, display_name):
        ...
        if resp.status_code == 200:
            print('\033[94m 今日头条: {} 发帖成功 \033[0m'.format(display_name))
            self.product.update_status(is_posted=1)
            return SUCCEED_STATUS, 'Succeed!'
        else:
            print('\033[94m 今日头条: {} 发帖失败 \033[0m'.format(display_name))
...
if __name__ == '__main__':
    ...
    # account = '18520293917'
    # password = 'cg19881004'
    # display_name = '手机用户69894429459'
    product = Product.objects.filter(~Q(images=''), is_posted=0)[0]
    TouTiao(account, product).main(account, password, display_name)
